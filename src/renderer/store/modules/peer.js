...
export default {
  ...
  getters: {
    ...
    broadcastPeers: (_, getters) => (networkId = null) => {
      const bestPeers = getters['bestPeers'](1, false, networkId)
      const randomPeers = getters['randomPeers'](1, networkId)
      const seedPeers = getters['randomSeedPeers'](1, networkId)
  ...
  actions: {
    ...
    async refresh ({ dispatch, getters, rootGetters }, network = null) {
      ...
      const networkLookup = {
        'qredit.mainnet': 'mainnet',
        'qredit.devnet': 'devnet'
      }
